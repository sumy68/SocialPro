...
export default function ConnectPlatformsScreen() {
  ...
  const handleConnect = async (platform: Platform) => {
    ...
    try {
      ...
      let redirectUriFromServer: string | undefined;
      ...
      try {
        ...
        redirectUriFromServer = initResult.redirectUri;
      } catch (initError: any) {
        console.log('[OAuth] Backend not available, using Demo Mode automatically');
        
        isDemoMode = true;
        authUrl = 'demo://auth';
        redirectUriFromServer = 'demo://callback';
      ...
      if (tokenData) {
        ...
        try {
          ...
          console.warn('[OAuth] Failed to save token to backend (expected in demo mode):', saveError.message);
        }
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              console.warn('[OAuth] Failed to disconnect from backend (expected in demo mode):', error.message);
            }
      ...
      {isConnecting ? (
        ...
        <TouchableOpacity
          style={[
            ...
            { backgroundColor: isConnected ? '#F3F4F6' : color },
          ]}
