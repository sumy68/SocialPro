...
export async function fetchPages(userToken: string): Promise<Array<{ id: string; name: string; access_token?: string }>> {
  const url = GRAPH + '/me/accounts?fields=id,name,access_token&access_token=' + encodeURIComponent(userToken);
  const res = await fetch(url);
  if (!res.ok) throw new Error('Fetch pages failed: ' + res.status + ' ' + res.statusText);
  const data = await res.json();
  return data.data || [];
}

/** Holt IG Business Account ID zu einer FB Page */
export async function fetchIgBusinessAccountId(pageId: string, userOrPageToken: string): Promise<string | null> {
  const url = GRAPH + '/' + pageId + '?fields=instagram_business_account&access_token=' + encodeURIComponent(userOrPageToken);
  const res = await fetch(url);
  if (!res.ok) throw new Error('Fetch IG business account failed: ' + res.status + ' ' + res.statusText);
  const data = await res.json();
